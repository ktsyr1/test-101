...
export default function Consultation() {
    ...
    let FormView = () => (
        <form className="flex w-full max-w-[400px] tap:mx-8 flex-col items-center " onSubmit={handleSubmit(onSubmit)}>
            ...
            <input type="submit" className="bg-safety-700 rounded-md mx-0 w-full  cursor-pointer hover:mx-1 p-3 my-2  max-w-[400px] " value={se} />
        </form >
    )
    let Dane = () => (
        <div className="mx-8 p-4 rounded-lg   flex w-full tap:max-w-[500px] flex-col items-center" >
            ...
            <div className="bg-white w-full max-w-[1000px] lap:max-w-[1360px] tap:m-20 m-2">
                <div className="border-4 border-safety-700 tap:rounded-xl tap:p-6 w-full p-3 rounded-none ">
                    <div className="bg-gradient-to-l from-[#0694A2] to-[#003035] rounded-xl p-4 text-white   flex flex-col tap:flex-row  justify-between items-center">
                        <h2 className="lap:text-4xl  p-2  w-full text-2xl text-center mb-4 tap:text-start tap:text-2xl  "> اشترك عبر البريد الإلكتروني <br />واحصل على استشارة مجانية </h2>
                        {forms == false ? <FormView /> : <Dane />}
                    </div>
                </div>
            </div>
        </>
