"use client"
import Icon, { IconArrow, IconClose } from "../icons"
import SizeBox from "../size-box"

import { createContext, useContext, useEffect, useState } from "react"
import { SubmitButtonProps, TypeReadmap, TypeSetup } from "./types";
    ...
    function Title({ data }: any) {
        ...
        return (
            <div className={`${_id.id === data.id && " !flex  "} tap:flex flex-row items-center hidden`}>
                <p className={`lap:text-2xl tap:text-lg   ${font} ${_id.id >= data.id ? "  font-bold  " : " "}`}  >{data.title}</p>
