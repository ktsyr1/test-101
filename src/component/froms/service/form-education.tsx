...
const options = ['بكالوريوس هندسة مدنية', 'بكالوريوس هندسة ميكانيكية', 'بكالوريوس هندسة كهربائية']
...
export default function FormEducation() {
    ...
    const onChange = () => {
        ...
        if (watch("codeEng")?.length > 3 && watch("yearsExperience") > 0) {
    ...
    const onSubmit = () => {
        let NewData = { codeEng: watch("codeEng"), yearsExperience: watch("yearsExperience"), study: study }
        console.log(NewData);
    ...
    function BtnsBol({ a }: any) {
        ...
        return (
            <button type="button" onClick={setS}
                className={`w-full rounded-md max-w-[300px] p-4 ml-10 bg-white flex flex-row justify-between ${study == a && "!bg-prussian-800 text-white"}`}
            >
                <p>{a} </p>
                {study == a ? <Icon.okBorder /> : <Icon.c01 size={20} />}
            </button>
        )
    }
    return (
        ...
        >
            <div className="flex flex-row w-full mt-20">
                <div className="flex flex-col mx-4 w-full">
                    ...
                    <input defaultValue="" {...register("codeEng", { require: true, minLength: 3 })} className='w-full p-2 rounded-md ml-4 ' minLength={3} placeholder=' أدخل رقم العضوية' />
                </div>
                <div className="flex flex-col mx-4  w-full">
                    <p className="text-xl  font-bold text-prussian-800 my-2">عدد سنوات الخبرة</p>
                    <input  {...register("yearsExperience", { require: true, max: 50 })} type='number' className='w-full p-2 rounded-md  ' minLength={3} placeholder='0' max={50} defaultValue={defaultData.yearsExperience} />
                </div>
            </div>
            <div className=" flex flex-row my-6"  >
                {options.map((a, i) => <BtnsBol a={a} key={i} />)}
            </div>
