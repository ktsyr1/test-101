...
export default function FormRef4() {
    ...
    const onSubmit: SubmitHandler<any> = (res) => {
        if (value?.length > 1) {
            setData({ ...data, isGoodGeneralLayout: value })
            setSelect(select + 1)
        }
    }
    let Select = ({ one, onClick, className }: any) => <div className={`p-4 rounded-full lap:text-base tap:text-sm text-xs  font-semibold ${value == one ? "bg-[#001D6C] text-white" : "text-[#001D6C] bg-white"} !pr-6 hover:shadow-lg ${className} `} onClick={onClick} >{one} </div>
    ...
    return (
        <Layout slug={4}>
            <form onSubmit={handleSubmit(onSubmit)} className="max-w-[1200px] tap:*:w-[45%] *:w-full *:m-4 *:p-2 *:rounded-lg  flex flex-wrap  justify-between mt-6" >
                ...
                {/* {list.map((a: any) => <Select value={value} one={a} key={a} onClick={() => setValue(a.value)} />)} */}
                {!value && <textarea className="!w-full" defaultValue={value} onChange={e => setValue(e.target.value)} placeholder="في موتمر او غيرها ..." />}
                <FormElm.Send />
            </form>
        </Layout>
    )
